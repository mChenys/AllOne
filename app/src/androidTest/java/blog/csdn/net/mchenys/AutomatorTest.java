...
/**
 * uiautomator 可以跨应用操作Ui
 */
@RunWith(AndroidJUnit4.class)
@SdkSuppress(minSdkVersion = 18)
public class AutomatorTest {

    private static final String PACKAGE_ESPRESSOTESTS = "blog.csdn.net.mchenys";
    ...
    public void testEspressoTestsApp() throws Exception {
        ...
        System.out.print("launcherPackage====" + launcherPackage);
        ...
        //通过id找到输入框一
        UiObject edt1 = mDevice.findObject(new UiSelector().resourceId(PACKAGE_ESPRESSOTESTS + ":id/editText")
                .className(EditText.class));
        //往里面输入字符2
        edt1.setText("2");
        //通过id找到输入框二
        UiObject edt2 = mDevice.findObject(new UiSelector().resourceId(PACKAGE_ESPRESSOTESTS + ":id/editText2")
                .className(EditText.class));
        //往里面输入5
        edt2.setText("5");
        //通过文本"计算"找到按钮
        ...
        btn.click();
        //通过id找到显示结果的textview
        UiObject tvResult = mDevice.findObject(new UiSelector().resourceId(PACKAGE_ESPRESSOTESTS + ":id/textView")
                .className(TextView.class));
        //判断结果与预期是否匹配
        assertEquals(tvResult.getText(), "计算结果：7");

        //通过文本"RecycleView"找到按钮
        UiObject btnRecycleView = mDevice.findObject(new UiSelector() .resourceId(PACKAGE_ESPRESSOTESTS + ":id/btn_to_rvactivity").className(Button.class));
        //执行点击事件跳转到另一个界面
        btnRecycleView.click();
        //通过id找到recycleview
        UiScrollable recycleview = new UiScrollable(new UiSelector()
                .className(RecyclerView.class)
                .resourceId(PACKAGE_ESPRESSOTESTS + ":id/recycleview"));
        //滑动到底部
    ...
    public void testSettingApp() throws Exception {
        ...
        UiObject dateTime = scrollview.getChild(new UiSelector().text("日期和时间").resourceId("com.android.settings:id/title"));
        dateTime.click();

        UiScrollable listView = new UiScrollable(new UiSelector().className(ListView.class).resourceId
                ("android:id/list"));
        UiObject use24Hours = listView.getChildByText(new UiSelector().className(TextView.class), "使用24小时格式");
        use24Hours.click();
